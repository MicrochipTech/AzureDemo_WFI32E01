...
void check_button_status(void)
{
    ...
    bool sw2_pressed = button_press_data.flag.sw2 == 1 ? true : false;
    ...
    if (sw1_pressed)
    {
        printf("AZURE: Button SW1 Count %u", button_press_data.sw1_press_count);
        RETURN_IF_FAILED(append_button_press_telemetry(&jw, event_name_button_sw1_span, button_press_data.sw1_press_count));
    }

    if (sw2_pressed)
    {
        printf("AZURE: Button SW2 Count %u", button_press_data.sw2_press_count);
        RETURN_IF_FAILED(append_button_press_telemetry(&jw, event_name_button_sw2_span, button_press_data.sw2_press_count));
    }
...
void check_led_status(twin_properties_t* twin_properties)
{
    ...
    printf("AZURE: %s() LED Status 0x%x", __func__, led_status.change_flag.as_uint16);
    ...
    az_span  response)
{
    ...
    printf("AZURE: Command Status: %u", status);
    ...
    uint16_t* out_response_status)
{
    ...
    *out_response_status = AZ_IOT_STATUS_SERVER_ERROR;
    ...
    {
        printf("AZURE: %s() : Payload %s", __func__, az_span_ptr(payload_span));
    ...
    uint16_t* out_response_status)
{
    ...
    *out_response_status = AZ_IOT_STATUS_SERVER_ERROR;
    ...
    {
        printf("AZURE: %s() : Payload Empty", __func__);
    }
    ...
    {
        debug_printError("AZURE: Failed to allocate memory for message string : Size %u", spanSize);
    ...
    twin_properties_t* twin_properties)
{
    ...
    if (az_result_succeeded(rc))
    {
        printf("AZURE: Property Topic   : %s", az_span_ptr(property_topic_span));
        printf("AZURE: Property Type    : %d", property_response.response_type);
        printf("AZURE: Property Payload : %s", (char*)payload);
    }
    else
    {
        printf("AZURE: Failed to parse property topic 0x%08x.", rc);
        printf("AZURE: Topic: '%s' Payload: '%s'", (char*)topic, (char*)payload);
        return rc;
    }
    ...
    twin_properties_t* twin_properties)
{
    ...
    if (twin_properties->flag.is_initial_get || twin_properties->flag.ip_address_updated != 0)
    {
        ...
        {
            printf("AZURE: Unable to add property for IP Address, return code  0x%08x", rc);
            return rc;
        }
    ...
    if (twin_properties->flag.app_property_1_updated != 0)
    {
        ...
        {
            printf("AZURE: Unable to add property for App MCU Property 1, return code  0x%08x", rc);
            return rc;
        }
    }
    ...
    if (twin_properties->flag.app_property_2_updated != 0)
    {
        ...
        {
            printf("AZURE: Unable to add property for App MCU Property 2, return code  0x%08x", rc);
            return rc;
        }
    }
